...
Class Heart_db{
	...
	*/
    ...
    if(is_array($this->param) && count($this->param) > 0 && isset($this->param['activate']) == TRUE)
      ...
      if(empty($this->config[$this->activate]['slaveOk']))
			{
				$this->slaveOk = false;
			}
			else
			{
				$this->slaveOk = $this->config[$this->activate]['slaveOk'];
			}
		}
		else
		{
	...
	*/
	public function aggregate($collection, $operation)
	{
        if (empty($collection))
	 	...
	 	try
	 	{
	 		$documents = $this->db->{$collection}->aggregate($operation,array('allowDiskUse' => true,));
	 		$this->_clear();
	 		if ($this->return_as == 'object')
			{
				return (object)$documents;
